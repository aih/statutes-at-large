/**
 * A per-route JavaScript byte budget (C5, ADR-0021), ported from the US Code
 * site's `jsbudget.test.ts` (its ADR-0046).
 *
 * `astro build` emits no client JavaScript for this site: every island is
 * `<script is:inline>`, which Astro passes through into the HTML verbatim and
 * never bundles. So the thing to measure is the inline script bytes each
 * route's component graph can ship, and those are in the `.astro` files —
 * counted from source, so the check runs in Vitest with no server and no
 * build.
 *
 * A component behind a false condition still contributes its script here:
 * the count is what a route *can* ship, not always what it did, which is the
 * safe direction for a ceiling. Where two page files serve one route, the
 * ceiling is the heavier page, since a request reaches one and never both.
 * Comments inside an inline script count, because Astro ships the script
 * verbatim and a reader downloads it.
 */
...
/** Every route the reader serves, walking `src/pages`. */
  ...
  it.each(measured)("$route ships at most its budget", ({ route, bytes, components }) => {
    ...
    expect(
      ...
      `${route} ships ${bytes} inline script bytes against a budget of ${budget}.\n` +
        ...
        `and say why in BUILDLOG.md — that edit is the record that it was a decision.`,
