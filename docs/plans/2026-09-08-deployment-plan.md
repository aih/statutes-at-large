...
$C comps poll --since 1990-01-01 --limit 400  # one slice; deploy/comps-walk.sh repeats it hourly until a run reports 0 new and 0 new versions; ~6 hours over the API's 1,000 calls an hour
```

`make load-all` is the first two lines; `make load-prod` runs the sequence
through the first COMPS run with the compose prefix, and `make comps-walk`
...
do not count toward the limit. `deploy/comps-walk.sh` repeats the slice
hourly as a transient unit:
...
A package that fails to load counts as fetched and fails again on the next
run, so the script ends the walk on a run whose summary line says `0 new, 0
new versions` (`ingest.comps.NOTHING_NEW`), after at most `MAX_RUNS` (48)
runs; the log is `${DATA_ROOT}/logs/comps-walk.log`. The first walk ended
with 10 packages that fail on every run, listed in the README. The site
